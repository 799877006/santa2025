...
# --- 核心树形类定义 ---
class ChristmasTree:
    def __init__(self, center_x='0', center_y='0', angle='0',shrink_factor=Decimal('0.999')):
    ...
    def _create_polygon(self):
        trunk_w = Decimal('0.15') * self.shrink_factor; trunk_h = Decimal('0.2') * self.shrink_factor  # 树干宽、高
        base_w = Decimal('0.7') * self.shrink_factor; base_y = Decimal('0.0') * self.shrink_factor     # 底层宽、y
        mid_w = Decimal('0.4') * self.shrink_factor; tier_2_y = Decimal('0.25') * self.shrink_factor   # 中层宽、y
        top_w = Decimal('0.25') * self.shrink_factor; tier_1_y = Decimal('0.5') * self.shrink_factor   # 上层宽、y
        tip_y = Decimal('0.8') * self.shrink_factor; trunk_bottom_y = -trunk_h * self.shrink_factor    # 树尖 y、树干底 y
...
def run_simulated_annealing(args):
    ...
    for i in range(effective_max_iter):  # 主循环
        ...
        if is_small_n:
            base_move_scale = max(0.005, 3.0 * (1 - progress))  # 小规模步长随进度缩小
            base_rotate_scale = max(0.001, 5.0 * (1 - progress))
        else:
            base_move_scale = max(0.001, 1.0 * (T / effective_t_start))  # 大规模步长随温度缩小
            base_rotate_scale = max(0.002, 5.0 * (T / effective_t_start))

        adaptive_factor = 1.0 / (1.0 + consecutive_failures * 0.5)
        move_scale = base_move_scale * adaptive_factor
        rotate_scale = base_rotate_scale * adaptive_factor
...
# --- 主流程 ---
def main():
    INPUT_CSV = "C:\\kaggle\\70.983188115078.csv"  # 初始输入
    OUTPUT_CSV = "C:\\kaggle\\70.983188115078_shrinked0.999.csv"        # 输出
